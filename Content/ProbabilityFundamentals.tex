...
\begin{frame}{System Reliability: Series vs. Parallel}
    We use the Multiplication Law of independent events to calculate the overall reliability ($R$) of an engineering system.
    ...
    \begin{block}{1. Series Mode (Logic AND)}
        ...
        \textit{Takeaway: Adding components in series always decreases overall reliability.}
    \end{block}
    
    \begin{block}{2. Parallel Mode (Redundancy)}
        Only \textbf{one} component needs to work. The system dies only if \textbf{all} components fail ($P_{fail} = 1 - R$). 
        ...
        \textit{Takeaway: Redundancy dramatically increases overall reliability.}
    \end{block}
\end{frame}
