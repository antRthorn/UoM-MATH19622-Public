%!TEX root = Reader.tex
\part{Probability Fundamentals}

\begin{frame}<presentation>[label=RecapDescriptiveStatistics]{Recap: Descriptive Statistics}
    Previously, we learned how to summarise historical data by separating the \textbf{Signal} from the \textbf{Noise}.

    \vspace{0.2cm}
    \begin{columns}[t]
        \column{0.48\textwidth}
        \begin{block}{1. Location (The Signal)}
            Where is the center of the data?
            \begin{itemize}
                \item \textbf{Mean:} The mathematical center.
                \item \textbf{Median:} The robust middle (ignores outliers).
            \end{itemize}
        \end{block}

        \column{0.48\textwidth}
        \begin{block}{2. Dispersion (The Noise)}
            How spread out is the data?
            \begin{itemize}
                \item \textbf{Variance/Std Dev:} The mathematical spread (remember $n-1$ for samples!).
                \item \textbf{IQR:} The robust middle 50\%.
            \end{itemize}
        \end{block}
    \end{columns}
  ...
  \vspace{0.4cm}
    ...
    Descriptive statistics let us understand the \textbf{past}. Next, use Probability to predict the \textbf{future}.
\end{frame}
...
\section{Outline of next block}

\only<article>{
    Welcome to Block 2. In the previous block, we learned how to mathematically describe data that we have already collected (the past). Now, we shift our focus to predicting the likelihood of events before they happen (the future). 
    
    Probability forms the absolute mathematical foundation of risk assessment, reliability engineering, signal processing, and system design. In this block, we will establish the fundamental rules and notation that govern random events.
    
In this block we will introduce the following concepts:
}
...
\only<article>{
    Probability forms the absolute mathematical foundation of risk assessment, reliability engineering, signal processing, and system design. In this part, we will establish the fundamental rules and notation that govern random events.
}
...
\begin{frame}[allowframebreaks]{The Sample Space and Events}
    ...
    \only<article>{
       We often consider probability experiments involving, flipping coins, rolling dice or drawing playing cards for illustration. However, In engineering, an experiment is a quality assurance test, a sensor reading, or a system failure check.
    }
...
\begin{frame}{The Addition Law (Unions)}
    ...
    \begin{block}{\only<presentation>{1. }For Mutually Exclusive Events}
    ...
    \begin{block}{\only<presentation>{2. }The General Addition Rule}
...
\begin{frame}{Visualising the Addition Law}
    ...
    \begin{tikzpicture}[scale=0.9, every node/.style={scale=0.9}]
        ...
        % --- Step 4: Visualise the overlap to subtract ---
        \only<beamer:4 | handout:1>{
            \draw[<-] (0,0) -- (1, -1.2) node[below] {\alert{Counted Twice!}};
        }

        % --- Step 5: Final Union (Hidden in Article mode) ---
        \only<beamer:5 | article:0 | handout:2>{
    ...
    % --- Article Reader Notes (Moved safely outside the TikZ picture) ---
    \only<article>{
        \vspace{0.3cm}
        \textbf{How to read this diagram:}
        \begin{itemize}
            \item Start with the entire area of Event A: $P(A)$.
            \item Add the entire area of Event B: $+P(B)$. 
            \item The intersection turns dark purple because we have counted it twice!
            \item To find the true Union area, we must subtract one instance of the intersection: $- P(A \cap B)$.
        \end{itemize}
    }
    
    % --- Instruction Block (appearing at the end in presentation mode) ---
    \only<presentation>{
        \pause[5] \vspace{-2mm}
...
\begin{frame}{A Simple Dice Example}
    \begin{example}[The 6-Sided Die]
        ...
        % Added 'onlytextwidth' to prevent the columns from overflowing the slide margins
        \begin{columns}[T, onlytextwidth] 
            % Expanded the left column slightly to give the text more breathing room
            ...
            % Shrunk the right column to tightly hug the QR code
            \begin{column}{0.2\textwidth} 
                \only<beamer>{
                    \centering
                    % Reduced the height from 2.5cm to 1.5cm
                    \qrcode[height=1.5cm]{https://www.menti.com/al4p5cwujeoh} \\
                    \vspace{0.1cm}
                    % Shrunk the text to the smallest legible size
                    \tiny Scan to answer!
                }
            \end{column}
        \end{columns}
...
\subsection{System Reliability: Series vs. Parallel}
\begin{frame}{System Reliability: Series vs. Parallel}
    \only<article>{We use the Multiplication Law of independent events to calculate the overall reliability ($R$) of an engineering system.}
    
    \begin{block}{1. Series Mode (Logic AND)}
        \textbf{All} components must work for the system to survive. We multiply the reliabilities directly:
        $$R_{sys} = R_1 \times R_2 \times R_3 \dots$$
      \only<article>{  \textit{Takeaway: Adding components in series always decreases overall reliability.}}
    \end{block}
    
    \begin{block}{2. Parallel Mode (Redundancy)}
        Only \textbf{one} component needs to work. \only<article>{The system dies only if \textbf{all} components fail ($P_{fail} = 1 - R$). }
        
        First, we multiply the failure probabilities:
        $$P_{fail\_total} = P_{fail\_1} \times P_{fail\_2} \times \dots$$
        Then, we find the new system reliability:
        $$R_{sys} = 1 - P_{fail\_total}$$
       \only<article>{ \textit{Takeaway: Redundancy dramatically increases overall reliability.}}
    \end{block}
\end{frame}
    ...
    \begin{example}[Drawing Socks without Replacement]
        ...
        \begin{columns}[T, onlytextwidth]
            ...
            \begin{column}{0.2\textwidth}
                \only<beamer>{
                    \centering
                    \qrcode[height=1.5cm]{https://www.menti.com/aleths3k2r1x} \\
                    \vspace{0.1cm}
                    \tiny Scan to answer!
                }
            \end{column}
...
% --- Summary Slide 1 ---
\begin{frame}{Summary: Probability Fundamentals}
    \begin{itemize}
        \item \textbf{The Language of Probability:} All experiments have a sample space ($S$). Events are subsets of that space.
        \vspace{0.2cm}
        \item \textbf{The Addition Law (Unions):} To find the probability of $A \cup B$, add their probabilities but remember to subtract the intersection!
        \[ P(A \cup B) = P(A) + P(B) - P(A \cap B) \]
        \vspace{0.1cm}
        \item \textbf{The Multiplication Law (Intersections):} To find the probability of $A \cap B$, multiply $P(A)$ by the conditional probability of $B$ given $A$.
        \[ P(A \cap B) = P(A) \times P(B|A) \]
        \vspace{0.1cm}
        \item \textbf{Independence:} If $A$ happening doesn't change the probability of $B$ happening, they are independent $\Rightarrow P(B|A) = P(B)$.
    \end{itemize}
\end{frame}

% --- Summary Slide 2 ---
\begin{frame}{Looking Ahead}
   \only<presentation>{ \vspace{1cm}}
    \begin{block}{Next Up: Discrete Probability Distributions}
        Now that we know the basic rules of random events, we will map these events to numerical variables. Instead of just looking at \lq Pass/Fail\rq\ events, we will learn how to calculate the probability of getting \textit{exactly 3 failures} in a batch of 50.
    \end{block}
\end{frame}
...
\input{Content/ProbabilityFundamentals_ExerciseQuestions.tex}
}