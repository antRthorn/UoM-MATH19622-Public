...
%
   \visible<3->{ {\emph{In this course, we mostly calculate Statistics to estimate Parameters.}\par}}
...
\end{example}
\only<beamer>{
    \normalsize Switch over to \textbf{menti.com} on your phones!
    
    \textit{(Code: 2860 3964)}
    }
...
%    \only<beamer>{
%    \framebreak
%     Switch over to \textbf{menti.com} on your phones!
%    
%    \textit{(Code: 2860 3964)}}
    ...
    \begin{example}
        ...
        \begin{itemize}
            ...
            \item \textbf{Is the coin biased?}              \pause Probably not. You just had \lq bad luck\rq\ (random variation) in your small sample.
        \end{itemize}
    \end{example}
...
\begin{frame}[allowframebreaks]{Ethics in Statistics: The \lq P-Hacking\rq\ Problem}
    ...
    \begin{example}[The (Un)ethical Engineer]
...
\begin{frame}{Where does Data come from?}
   ...
   \visible<3->{ {\centering \textit{In both cases, we use \textbf{samples} (small datasets) to estimate the properties of the whole.}\par}}
...
\subsection{Visualising Data: Bar Charts vs. Histograms}

\begin{frame}{Visualising Data: Bar Charts vs. Histograms}
    A very common mistake in engineering is confusing Bar Charts with Histograms. The type of chart you use depends entirely on the type of data you have!

    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{block}{\only<presentation>{1. }Bar Chart (Discrete / Categorical)}
            \begin{itemize}
                \item Used for distinct categories or countable integers.
                \item There are \textbf{gaps} between the bars because the values do not flow into each other.
                \item \textit{Example:} Number of faults per machine (Machine A, Machine B).
            \end{itemize}
        \end{block}

        \column{0.5\textwidth}
        \begin{block}{\only<presentation>{2. }Histogram (Continuous)}
            \begin{itemize}
                \item Used for continuous measurements.
                \item The x-axis is a continuous number line grouped into \lq bins\rq.
                \item There are \textbf{no gaps} between the bars. The \textit{area} represents frequency.
                \item \textit{Example:} The distribution of battery voltages (4.9V to 5.1V).
            \end{itemize}
        \end{block}
    \end{columns}
    ...
    \only<article>{
        \vspace{0.4cm}
        \textbf{The Engineering Connection:} \\
        Histograms are incredibly powerful for visualising the \lq shape\rq\ of your noise. If you measure the voltage of 10 batteries, your histogram bins will be wide and blocky. However, if you measure 10,000 batteries and make your bins infinitely narrow, the jagged top of the histogram smooths out into a continuous curve. We will revisit this exact concept in Block IV when we learn about the Normal Distribution (the Bell Curve)!
    }
\end{frame}
    ...
    \begin{recipe}[Calculating the Median]
        \begin{enumerate}
            ...
            \begin{itemize}
                ...
                \item If $n$ is \textbf{even}: The median is the mean of the two middle numbers.
            \end{itemize}
        \end{enumerate}
    \end{recipe}
...
% --- LIVE LECTURE VERSION ---
\begin{frame}<beamer>[fragile]{Interactive Demonstration}
    ...
    \vspace{1cm}
    \centering
    \href{run:./code/run_MeanVersusMedian.command}{\beamerbutton{\Large Launch Python Simulation}}
...
\begin{frame}<handout>[fragile]{Interactive Demonstration}
    \structure{The Pull of the Outlier}
    ...
    To see this interactive simulation, download the course code folder from \textbf{Canvas}, open it in VS Code, and run the following command in your terminal:
    
    \begin{lstlisting}[language=bash]
uv run code/MeanVersusMedian.py
    \end{lstlisting}
    
    \vspace{0.2cm}
    \textit{This will launch a GUI with sliders, allowing you to manually change the latency values and watch how the Mean and Median react.}
\end{frame}
    ...
    \only<article>{
        ...
        \begin{example}[Calculating the IQR]
            ...
            \begin{itemize}
                ...
                \item \textbf{The IQR:} $18 - 12 = \mathbf{6^\circ\text{C}}$ \textit{(Accurately describes the typical behaviour)}
            \end{itemize}
   ...
   \begin{solution}
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{ccc}
            ...
            9 & +4 & 16 \\
            \midrule
            \textbf{Sum} (Step 4)& \textbf{0} & \textbf{32} \\
            \bottomrule
        \end{tabular}
      
    \end{table}
Step 5: $ s^2 = \frac{32}{8-1} = \frac{32}{7} = 4.57 $ 

Step 6: $ s = \sqrt{4.57} \approx 2.14 $
    \end{solution}
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{block}{Standard Deviation ($\sigma$)}
            ...
            \begin{itemize}
                ...
                \item \textbf{Pro:} Intuitive. Describes the \lq average distance\rq\ from the mean.
...
\input{Content/IntroductionAndDataTypes_ExerciseQuestions.tex}
}
