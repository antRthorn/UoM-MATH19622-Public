...
\begin{frame}{Mean and Variance of Continuous Distributions}
  \only<article>{  Just as we did for discrete distributions, we can find the centre and spread of a continuous probability density function (PDF). The summations ($\sum$) simply become integrals ($\int$) over the entire domain!}
   ...
   \only<article>{     \textbf{The Shortcut Formula:} Just like the discrete case, it is usually much easier to calculate using $E(X^2)$:}
    ...
    \only<article>{
        \vspace{0.3cm}
        \textit{Instructor Note: Remind the students that standard deviation ($\sigma$) is still just the square root of the variance. The shortcut formula is especially important here, as expanding $(x-\mu)^2$ inside an integral usually leads to a messy and error-prone integration by parts!}
    }
\end{frame}
...
\begin{frame}[allowframebreaks]{Example: Comparing Apples to Oranges}
    ...
    \begin{example}[Quality Control on Two Lines]
        ...
        \structure{Question:} Which of these two components is more "unusual" (i.e., relatively further from its design specification)?
    \end{example}
...
\begin{frame}{The Standard Normal Table ($Z$-Table)}
  ...
  \includegraphics[trim=2cm 2.5cm 2cm 8.25cm, clip, height=0.95\textheight, keepaspectratio]{Resources/NormalDistibutionTable.pdf}
    ...
    \only<article>{
        \textit{Instructor Note: For example, to find $Z = 1.24$, follow the row for 1.2 across to the column for 0.04. The value is 0.8925.}
    }
\end{frame}
...
\begin{frame}{Solution: Using the Z-Table (1/2)}
    \begin{solution}
        ...
        \begin{itemize}
            ...
            \item We want $P(Z > 1.25)$. The table only gives "less than" areas!
...
\begin{frame}{Solution: Using the Z-Table (2/2)}
    ...
    \only<article>{
        \vspace{0.4cm}
        \textbf{Instructor Note:} Always encourage students to draw a quick sketch of the bell curve for these problems! Shading the area they are looking for is the single best way to avoid accidentally forgetting the "$1 - $" step for "greater than" or negative Z-score problems.
    }
\end{frame}
...
\begin{frame}{Beyond the Normal Curve}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item \textit{Used in:} Modeling quantization error in Analog-to-Digital Converters (ADCs).
        \end{itemize}
    ...
    \only<article>{
        \vspace{0.3cm}
        \textit{Instructor Note: Reassure the students that they don't need to memorize these for the current exam! This is purely to show them that the exact same mechanics they just learned (integrating the PDF to find the CDF and the mean) apply perfectly to all of these advanced distributions.}
    }
\end{frame}
