...
\begin{frame}[label=RecapProbabilityFundamentals]{Recap: Probability Fundamentals}
...
\begin{frame}[label=RecapDiscreteDistributions]{Recap: Discrete Distributions}
...
\begin{frame}{The \lq Exact Point\rq\ Paradox}
    \begin{example}[The infinitely precise resistor]
        ...
        \begin{minipage}{0.2\textwidth}
            \centering
            \qrcode[height=1.2cm]{https://www.menti.com/al46dc9t7s3m} \\
            \vspace{0.1cm}
            \scriptsize \textbf{Scan to play!}
        \end{minipage}
    \end{example}
...
\begin{frame}{Worked Example: A Custom Continuous Distribution}
    ...
    % 1. Safely define the width variable!
    \def\boxwidth{\textwidth} % Default width for handout/article modes
    \mode<beamer>{\def\boxwidth{0.85\textwidth}} % Overwrite it for the projector
    
    \begin{example}[Sensor Calibration Error]
        % 2. Use our clean variable here
        \begin{minipage}{\boxwidth}
            ...
            \end{enumerate}
        \end{minipage}%
        % 3. The QR code safely appears ONLY during the live presentation
        \only<beamer>{%
        \hfill
...
\begin{frame}{Solution: A Custom Continuous Distribution (1/5)}
    \begin{solution}
...
\begin{frame}{Solution: A Custom Continuous Distribution (2/5)}
    \begin{solutioncont}
...
\begin{frame}{Solution: A Custom Continuous Distribution (3/5)}
    \begin{solutioncont}
...
\begin{frame}{Solution: A Custom Continuous Distribution (4/5)}
    \begin{solutioncont}
        ...
        \begin{column}{0.5\textwidth}
            \begin{block}{Properties}
                \begin{itemize}
                    ...
                    \item \textbf{Centred:} Mean = Median = Mode = $\mu$.
...
\only<beamer>{\small}
    ...
    \only<beamer>{
        ...
        \begin{center}
            \qrcode[height=0.7cm]{https://www.menti.com/al46dc9t7s3m} \\
            \vspace{0.1cm}
            \scriptsize \textbf{Scan to answer!}
        \end{center}
    }
\end{frame}
...
\begin{frame}{Worked Example: Calculating Probabilities}
    ...
    \only<beamer>{\pause \vspace{0.2cm}}

    % Safely displays the QR code at the bottom of the slide ONLY on the projector
    \only<beamer>{
        \begin{center}
            \qrcode[height=1.1cm]{https://www.menti.com/al46dc9t7s3m} \\
            \vspace{0.1cm}
            \scriptsize \textbf{Scan to answer!}
        \end{center}
    }
...
\begin{frame}{Example: The Empirical Rule}
    ...
    % Safely groups the pause, spacing, and QR code to ONLY appear on the projector
    \only<beamer>{
        \pause 
        \begin{center}
            \qrcode[height=1cm]{https://www.menti.com/al46dc9t7s3m} \\
            \vspace{0.1cm}
            \scriptsize \textbf{Scan to answer!}
        \end{center}
    }
...
\begin{frame}[allowframebreaks]{Example: Poisson to Normal}
    ...
    % Safely displays the QR code on the projector without breaking the framebreaks!
    \only<beamer>{
        \vspace{0.2cm}
        \begin{center}
            \qrcode[height=1.2cm]{https://www.menti.com/al46dc9t7s3m} \\
...
\subsection{Is everything \lq normal\rq\ }
\begin{frame}{A Crucial Warning: The Central Limit Theorem}
    Wait, does \textit{every} distribution become a bell curve if we just collect enough data?
    
    \vspace{0.2cm}
    \begin{alertblock}{The Trap}
        \textbf{No!} If you roll a fair 6-sided die 1,000,000 times, the distribution of those individual rolls stays perfectly flat (a Uniform Distribution).
    \end{alertblock}

    \pause
    \vspace{0.2cm}
    \begin{block}{The Central Limit Theorem (CLT)}
        The Normal curve only appears when we take the \textbf{SUM} or the \textbf{AVERAGE} of many independent random variables.
        
        \begin{itemize}
            \item \textbf{Why Binomial works:} \only<article>{A Binomial variable is exactly this, It }is the \textit{sum} of $n$ independent binary successes\only<article>{ (like counting the total number of defective chips)}. 
            \item Because $n$ is so large, we \only<article>{are summing hundreds of independent events, which causes the final total to }naturally form a Normal Distribution.
        \end{itemize}
    \end{block}
\end{frame}
...
\input{Content/ContinuousDistributions_ExerciseQuestions.tex}
}
