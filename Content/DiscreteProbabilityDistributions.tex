...
\only<article>{ In the previous block, we looked at the probability of specific, single events (like drawing a red sock). In engineering, we usually want to model a whole system of events mathematically.  To do this, we use \textbf{Random Variables}.}
    ...
    \begin{example}[Component Testing]
        \begin{columns}[T, onlytextwidth]
            ...
            \begin{column}{0.2\textwidth}
                \only<beamer>{
                    \centering
                    \qrcode[height=2cm]{https://www.menti.com/alajfwe4w467} \\
                    \vspace{0.1cm}
                    \scriptsize Scan to answer!
                }
            \end{column}
        \end{columns}
    \end{example}
...
\begin{frame}[allowframebreaks]{The \lq 8 Heads\rq\ Problem Solved}
    \begin{example}[Is the coin biased?]
        ...
        \begin{columns}[T, onlytextwidth]
            ...
            \begin{column}{0.2\textwidth}
                \only<beamer>{
                    \centering
                    \qrcode[height=1.5cm]{https://www.menti.com/alajfwe4w467} \\
                    \vspace{0.1cm}
                    \tiny Scan to answer!
                }
            \end{column}
    ...
    \begin{solutioncont}
        ...
        \vspace{0.4cm}
        \textit{(Note: We will learn exactly where to draw the \lq impossible\rq\ line when we cover Hypothesis Testing in Block 5!)}
    \end{solutioncont}
\end{frame}
...
\begin{frame}[fragile]{Python: The Binomial Distribution}
    ...
    \only<article>{
    When using the \texttt{scipy.stats} library to calculate discrete probabilities, the \texttt{pmf} (Probability Mass Function) command requires three core arguments to map exactly to our mathematical formula:
    
    \begin{description}
        \item[\texttt{k = 8}:] The target number of \lq successes\rq\ you want to evaluate (often denoted as $x$ in our mathematical formulas). In this case, exactly 8 heads.
        \item[\texttt{n = 10}:] The total number of independent trials. Here, we are flipping the coin 10 times.
        \item[\texttt{p = 0.5}:] The probability of success on a single, isolated trial (a 50\% chance of getting heads on any given flip).
    \end{description}
    
If you ever need to find the cumulative probability—such as the probability of getting \textbf{8 or fewer} heads ($P(X \leq 8)$)—you simply swap \texttt{.pmf} for \texttt{.cdf} (Cumulative Distribution Function), leaving the three arguments exactly the same.
}
\end{frame}
...
\begin{frame}[allowframebreaks]{Engineering Example: Network Faults}
    \begin{example}[Fibre Optic Cable Faults]
        A long-haul fibre optic network experiences an average of $\lambda = 2$ dropped connections per day. 
...
\begin{frame}[fragile]{Python: The Poisson Distribution}
 ...
 \only<article>{
    When modelling the Poisson distribution using \texttt{scipy.stats}, the syntax is very similar to the Binomial setup, but the arguments change to reflect the Poisson parameters:
    
    \begin{description}
        \item[\texttt{k}:] The specific number of events you are checking (e.g., \texttt{0} drops, or up to \texttt{2} drops). This corresponds to $x$ in our mathematical formulas.
        \item[\texttt{mu} ($\lambda$):] The expected average rate of occurrence. In this example, the historical average is \texttt{2} drops. \textit{Note: SciPy uses the argument name \texttt{mu} instead of $\lambda$ because \texttt{lambda} is a reserved mathematical keyword in the Python language!}
    \end{description}
    
        \textbf{Which function do you use?}
    \begin{itemize}
        \item \textbf{\texttt{.pmf(0, 2)}:} The \textbf{Probability Mass Function} calculates the probability of getting \textit{exactly} $k=0$ drops.
        \item \textbf{\texttt{.cdf(2, 2)}:} The \textbf{Cumulative Distribution Function} calculates the probability of getting \textit{up to and including} $k=2$ drops. It automatically does the heavy lifting of summing $P(X=0) + P(X=1) + P(X=2)$.
    \end{itemize}

    \textit{Comment on Output Formatting:} The \texttt{f"\{p\_0:.4f\}"} syntax in the print statement is a Python f-string. The \texttt{:.4f} command tells Python to automatically round the output to exactly \textbf{4 decimal places} (floating-point numbers), which is the standard reporting format for probabilities.
}
  
\end{frame}

\section{Summary: Discrete Distributions}
\begin{frame}{Summary: Discrete Distributions}
    \small
    \begin{itemize}
        \item \textbf{Random Variables ($X$)}
            \begin{itemize}
                \item Maps sample spaces to numerical values.
                \item \textbf{Expectation:} $E(X) = \sum x P(x)$ (The long-run average).
            \end{itemize}
        \vspace{0.2cm}
        \item \textbf{The Binomial Model}
            \begin{itemize}
        ...
        \item \textbf{The Poisson Distribution}
            \begin{itemize}
                \item Modelling rare events over a continuous interval (time/space).
                \item Formula: $P(X=k) = \frac{e^{-\lambda} \lambda^k}{k!}$
                \item Mean and Variance are identical: $\mu = \sigma^2 = \lambda$.
            \end{itemize}
    \end{itemize}

    \only<presentation>{\pause \vspace{-0.3cm}}

    \begin{block}{Next Up: Continuous Probability Distributions}
        \only<article>{So far, we have only counted discrete \lq events\rq\ (like 3 broken bulbs or 2 dropped packets).} But what happens when our random variable is a continuous physical measurement\only<article>{, like the exact voltage of a battery or the thermal noise in a circuit}? \only<article>{Next, we introduce the famous Normal Curve.}
    \end{block}
\end{frame}

\only<article>{
\section{Exercise Questions}
\input{Content/DiscreteProbabilityDistributions_ExerciseQuestions.tex}
}